...
{	
	...
	meshList[GEO_SPHERE] = MeshBuilder::GenerateSphere("sphere", Color(1, 0, 0), 30, 40);
	...
	meshList[GEO_LIGHTBALL] = MeshBuilder::GenerateSphere("sphere", Color(1, 1, 1), 10, 20);
	meshList[GEO_QUAD] = MeshBuilder::GenerateQuad("quad", Color(1, 1, 1), 1.f);
	meshList[GEO_QUAD]->textureID = LoadTGA("Image//color.tga");
	meshList[GEO_QUAD]->material.kAmbient.Set(0.2f, 0.2f, 0.2f);
	meshList[GEO_QUAD]->material.kDiffuse.Set(0.5f, 0.5f, 0.5f);
	meshList[GEO_QUAD]->material.kSpecular.Set(1.0f, 1.0f, 1.0f);
	...
	meshList[GEO_MODEL3] = MeshBuilder::GenerateOBJ("model3", "OBJ//winebottle.obj");
	meshList[GEO_MODEL3]->textureID = LoadTGA("Image//winebottle.tga");
...
void Scene14::RenderSkybox()
{
	const float OFFSET = 499;
	// front
	modelStack.PushMatrix();
	modelStack.Translate(camera.position.x, camera.position.y, -OFFSET+camera.position.z);
	...
	//left
	modelStack.PushMatrix();
	modelStack.Translate(-OFFSET+camera.position.x, camera.position.y, camera.position.z);
	...
	//top
	modelStack.PushMatrix();
	modelStack.Translate(camera.position.x, OFFSET+camera.position.y, camera.position.z);
	modelStack.Rotate(-90, 0, 1, 0);
	modelStack.Rotate(90, 1, 0, 0);
	modelStack.Scale(1000, 1000, 1000);
...
void Scene14::RenderMesh(Mesh* mesh, bool enableLight)
{
	Position lightPosition_cameraspace = viewStack.Top() * light[0].position;
	glUniform3fv(m_parameters[U_LIGHT0_POSITION], 1, &lightPosition_cameraspace.x);
...
void Scene14::RenderText(Mesh* mesh, std::string text, Color color)
{
...
void Scene14::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
	if (!mesh || mesh->textureID <= 0) //Proper error check
		return;
