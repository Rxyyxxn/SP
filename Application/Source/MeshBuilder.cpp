...
Mesh* MeshBuilder::GenerateQuad(const std::string &meshName, Color color, float length)
{
	...
	v.pos.Set(0.5f * length, 0.5f * length, 0.f);	v.color = color;	v.normal.Set(0, 0, 1);	v.texCoord.Set(1, 1);	vertex_buffer_data.push_back(v);
	v.pos.Set(-0.5f * length, 0.5f * length, 0.f);	v.color = color;	v.normal.Set(0, 0, 1);	v.texCoord.Set(0, 1);	vertex_buffer_data.push_back(v);
	v.pos.Set(-0.5f * length, -0.5f * length, 0.f);	v.color = color;	v.normal.Set(0, 0, 1);	v.texCoord.Set(0, 0);	vertex_buffer_data.push_back(v); // bottom right
	v.pos.Set(0.5f * length, -0.5f * length, 0.f);	v.color = color;	v.normal.Set(0, 0, 1);	v.texCoord.Set(1, 0);	vertex_buffer_data.push_back(v); // 
...
Mesh* MeshBuilder::GenerateSemiCircle(const std::string& meshName, Color color, unsigned numSlices, float radius)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
...
Mesh* MeshBuilder::GenerateHemisphere(const std::string& meshName, Color color, unsigned numStacks, unsigned numSlices, float radius)
{
	...
	for (unsigned slice = 0; slice < numSlices + 1; ++slice)
	{
		...
		v.pos.Set(x, 0, z);		v.color;	v.normal.Set(0, -1, 0);		vertex_buffer_data.push_back(v);
		v.pos.Set(0, 0, 0);		v.color;	v.normal.Set(0, -1, 0);		vertex_buffer_data.push_back(v);
	}
	for (unsigned slice = 0; slice < numSlices + 1; ++slice)
	{
		index_buffer_data.push_back(startIndex + slice * 2 + 1);
...
Mesh* MeshBuilder::GenerateTorus(const std::string& meshName, Color color, unsigned numSlices, unsigned numStacks, float OuterRadius, float InnerRadius)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;

	float radianPerStack = Math::HALF_PI / numStacks;
	...
	for (size_t stack = 0; stack < numStacks + 1; stack++)
	{
		for (unsigned slice = 0; slice < numSlices + 1; slice++)
		{
			z1 = OuterRadius * cos(stack * radianPerStack);

			x1 = OuterRadius * sin(stack * radianPerStack);

			z2 = (OuterRadius + InnerRadius * cos(slice * radianPerSlice)) * cos(stack *

				radianPerStack);

			y2 = InnerRadius * sin(slice * radianPerSlice);

			x2 = (OuterRadius + InnerRadius * cos(slice * radianPerSlice)) * sin(stack *
				radianPerStack);

			v.pos.Set(x2, y2, z2);
			v.color = color;
			v.normal.Set(x2 - x1, y2, z2 - z1);
			v.normal.Normalize(); vertex_buffer_data.push_back(v);
		}
	}
...
Mesh* MeshBuilder::GenerateText(const std::string& meshName, unsigned numRow, unsigned numCol)
{
	...
	for (unsigned row = 0; row < numRow; ++row)
	{
		...
		{
			v.pos.Set(0.5f, 0.5f, 0.f);		v.normal.Set(0, 0, 1);	v.texCoord.Set((col + 1.f) * width, (numRow - row) * height);	vertex_buffer_data.push_back(v); //v0
			...
			v.pos.Set(0.5f, -0.5f, 0.f);	v.normal.Set(0, 0, 1);	v.texCoord.Set((col + 1.f) * width, (numRow - row - 1) * height);	vertex_buffer_data.push_back(v); //v3
...
Mesh* MeshBuilder::GenerateCube(const std::string& meshName, Color color, float length)
{
	...
	v.pos.Set(-0.5f * length, -0.5f * length, -0.5f * length);	v.color = color; v.normal.Set(-1, 0, 0);	vertex_buffer_data.push_back(v);//-x
	v.pos.Set(-0.5f * length, -0.5f * length, 0.5f * length);	v.color = color; v.normal.Set(-1, 0, 0);	vertex_buffer_data.push_back(v);//-x
	v.pos.Set(-0.5f * length, 0.5f * length, 0.5f * length);	v.color = color; v.normal.Set(-1, 0, 0);	vertex_buffer_data.push_back(v);//-x
	v.pos.Set(0.5f * length, 0.5f * length, -0.5f * length);	v.color = color; v.normal.Set(0, 0, -1);	vertex_buffer_data.push_back(v);//-z
	v.pos.Set(-0.5f * length, -0.5f * length, -0.5f * length);	v.color = color; v.normal.Set(0, 0, -1);	vertex_buffer_data.push_back(v);//-z
	v.pos.Set(-0.5f * length, 0.5f * length, -0.5f * length);	v.color = color; v.normal.Set(0, 0, -1);	vertex_buffer_data.push_back(v);//-z

	v.pos.Set(0.5f * length, -0.5f * length, 0.5f * length);	v.color = color; v.normal.Set(0, -1, 0);	vertex_buffer_data.push_back(v);//-y
	v.pos.Set(-0.5f * length, -0.5f * length, -0.5f * length);	v.color = color; v.normal.Set(0, -1, 0);	vertex_buffer_data.push_back(v);//-y
	v.pos.Set(0.5f * length, -0.5f * length, -0.5f * length);	v.color = color; v.normal.Set(0, -1, 0);	vertex_buffer_data.push_back(v);//-y
	v.pos.Set(0.5f * length, 0.5f * length, -0.5f * length);	v.color = color; v.normal.Set(0, 0, -1);	vertex_buffer_data.push_back(v);//-z
	v.pos.Set(0.5f * length, -0.5f * length, -0.5f * length);	v.color = color; v.normal.Set(0, 0, -1);	vertex_buffer_data.push_back(v);//-z
...
Mesh* MeshBuilder::GenerateCone(const std::string& meshName, Color color, unsigned numSlices, float radius, float height)
{
	...
	for (unsigned slice = 0; slice < numSlices + 1; ++slice)
	{
		...
		v.pos.Set(radius * x, -height * 0.5, radius * z);		v.color = color; v.normal.Set(height * x, radius, height * z); v.normal.Normalize();	vertex_buffer_data.push_back(v);
		v.pos.Set(0, height * 0.5, 0);		v.color = color;	v.normal.Set(x, 0, z);	vertex_buffer_data.push_back(v);
	}
	for (unsigned slice = 0; slice < numSlices + 1; ++slice)
	{
		index_buffer_data.push_back(startIndex + slice * 2 + 0);
